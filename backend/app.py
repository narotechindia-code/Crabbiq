...
def validate_url(value: str) -> str:
    if len(value) > MAX_URL_LENGTH:
        raise HTTPException(status_code=413, detail="URL is too long")
    parsed = urlparse(value)
    if parsed.scheme.lower() not in ALLOWED_SCHEMES or not parsed.netloc:
        raise HTTPException(status_code=400, detail="Only absolute HTTP(S) URLs are supported")
    # Explicitly reject localhost and obvious loopback targets from the public resolver.
    hostname = (parsed.hostname or "").lower().rstrip(".")
    blocked = {"localhost", "127.0.0.1", "::1", "0.0.0.0"}
    if hostname in blocked or hostname.endswith(".localhost"):
        raise HTTPException(status_code=400, detail="Local destinations are not accepted")
    return value
...
def pick_format(mode: str, quality: str, audio_format: str) -> tuple[str, dict[str, Any]]:
    if mode == "audio" or quality == "audio":
        ...
        return "bestaudio/best", {"postprocessors": [post]}

    # Image mode uses best thumbnail/direct image when the extractor exposes it.
    if mode == "image":
        return "best", {"writethumbnail": True}
    ...
    try:
        ...
        def progress_hook(data: dict[str, Any]) -> None:
            ...
            if status == "downloading":
                ...
                set_job(job_id, progress=pct, speed=data.get("speed"), eta=data.get("eta"))
            elif status == "finished":
                set_job(job_id, progress=100)

        opts = extractor_options(cookie_browser=payload.cookies_from_browser, download=True, outtmpl=outtmpl)
        opts.update(extra)
        opts["format"] = fmt
        ...
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            if not existing:
                ...
                existing = candidates
            if not existing:
                raise RuntimeError("Download completed without a produced file")
            primary = existing[0]
...
def health() -> dict[str, Any]:
    return {"ok": True, "service": APP_NAME, "active_limit": MAX_ACTIVE}


@app.post("/api/inspect")
def inspect(payload: InspectRequest) -> dict[str, Any]:
    url = validate_url(str(payload.url))
    opts = extractor_options(cookie_browser=payload.cookies_from_browser)
    try:
        with yt_dlp.YoutubeDL(opts) as ydl:
        ...
        return {
            ...
            "is_playlist": bool(info.get("_type") == "playlist"),
            "formats": formats,
        }
    except Exception as exc:
        raise HTTPException(status_code=422, detail={"message": "Unable to resolve this URL", "reason": str(exc)[:1000]}) from exc
...
def start_download(payload: DownloadRequest, background_tasks: BackgroundTasks) -> dict[str, Any]:
    ...
    with _jobs_lock:
        _jobs[job_id] = {"id": job_id, "status": "queued", "progress": 0, "created_at": asyncio.get_event_loop().time()}
...
def job_status(job_id: str) -> dict[str, Any]:
    ...
    if job.get("status") == "completed":
        safe["download_url"] = f"/api/jobs/{job_id}/file"
    return safe
...
def download_file(job_id: str, request: Request) -> FileResponse:
    ...
    if not job or job.get("status") != "completed":
        raise HTTPException(status_code=404, detail="Completed file not found")
    path = Path(job["path"]).resolve()
    if DOWNLOAD_DIR not in path.parents:
        raise HTTPException(status_code=500, detail="Invalid file location")
    if not path.is_file():
